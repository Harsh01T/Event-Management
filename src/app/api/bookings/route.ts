...
export async function POST(req: Request) {
  try {
    const { userId, eventId, ticketCount } = await req.json();

    const booking = await prisma.$transaction(async (tx) => {
      const event = await tx.event.findUnique({ where: { id: eventId } });
      if (!event || event.remainingTickets < ticketCount) throw new Error("Sold out");
      ...
      return await tx.booking.create({
        data: { userId, eventId, ticketCount, uniqueCode: uuidv4().slice(0,8).toUpperCase() }
      });
    });
